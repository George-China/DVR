...
//    ifmt_ctx_->iformat
    ...
    // 1.打开和解析文件 函数内部实现封装格式的识别，  yuv、pcm raw 需要指定 AVInputFormat *fmt
    ...
    //rtmp rtsp  flv
    ret = avformat_find_stream_info(ifmt_ctx_, NULL);  // 里面还调用解码器去做解码尝试， 怎么知道做了decode，比如
    if(ret < 0) {
        av_strerror(ret, err2str, sizeof(err2str));
    ...
    // 获取stream index 通过av_find_best_stream
...
void DemuxThread::Run()  // 对应的线程业务
{
    ...
    while (abort_ != 1) {
        // 如果最多缓存10秒 的数据，怎么计算队列的大小
        if(audio_queue_->Size() > 100 || video_queue_->Size() > 100)
        {
            std::this_thread::sleep_for(std::chrono::milliseconds(10));
            continue;
        }
        // pkt 在调用av_read_frame的时候，音频或视频数据被先释放掉，然后重新分配新的buffer
        ret = av_read_frame(ifmt_ctx_, &pkt); //  fread
        if(ret < 0) {
            av_strerror(ret, err2str, sizeof(err2str));
